...
def get_data_from_db():
    ...
    retail = pd.DataFrame(result_retail)
                                # แก้ไข datetime เพื่อ upload เข้า bigquery
...
default_args = {
    ...
    'start_date': days_ago(1),  #เพื่อไม่ให้รันซ้ำอีกครั้ง
    ...
    # cause load have to schedule_interval | @once = 1 
}
...
t4 = BashOperator(
    ...
    bash_command='bq load --source_format=CSV --autodetect\
         road_to_dataengineer.online_retail1 gs://asia-northeast1-roaddata-8846ce11-bucket/data/result.csv',
    dag=dag,
)
